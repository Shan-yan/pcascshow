...
The enlarged landing page makes the benchmark name, Chinese academic subtitle
...
transition gradient sits behind the images so it cannot cross an enlarged tile.
The next section animates the transition from
task-success-only evaluation to public-space safety and PCA-SC diagnosis.
One large circular-text track combines 300 instances, six evaluated models and
the 50+ participant human study around a luminous, live-animated “Start
exploring / 开始探索” control. Selecting it expands a full-width five-card Bento
...
pointer-position spotlight cards and click sparks—but is reimplemented locally
in React and CSS. All effects respect
`prefers-reduced-motion`.

The model-result pages use a locally bundled, npm-installed Apache ECharts radar
component for all six model portraits. Radar axes remain on the reported 0–1
scale; the displayed consistency axis is the mean of PC, CA and PA for visual
summary only and is explicitly not an overall benchmark score.

The shield-shaped website emblem in `public/brand/` was generated from the
author-provided visual reference and contains no embedded text. It is bundled
locally with the static release.
