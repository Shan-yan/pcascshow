    ...
    <>
      <section className="hero">
        <div className="container hero__grid">
          ...
          <div className="system-map" aria-label="PCA-SC 评测逻辑">
            <div className="system-map__head"><span>评测逻辑</span><span>仅使用可观察证据</span></div>
            <div className="system-map__input"><span className="system-map__index">01</span><div><strong>公共空间场景</strong><small>图像 · 任务 · 四个候选动作</small></div></div>
            <div className="system-map__flow">{["感知", "认知", "行动"].map((item, index) => <div className="system-node" key={item}><span>0{index + 2}</span><strong>{item}</strong></div>)}</div>
            <div className="system-map__outputs"><div><i className="dot dot--blue" />任务成功</div><div><i className="dot dot--green" />安全表现</div><div><i className="dot dot--amber" />链路一致性</div></div>
            <div className="system-map__foot"><Icon name="shield" /><span>任务完成与安全表现作为独立目标报告。</span></div>
          </div>
        </div>
      </section>
      ...
      <section className="section section--soft">
        <div className="container">
          ...
          <div className="motivation-flow">
            <div className="motivation-step"><span>传统问题</span><h3>任务完成了吗？</h3><p>单一成功率无法说明任务以什么方式完成。</p></div>
            <div className="motivation-arrow"><Icon name="arrow" /></div>
            <div className="motivation-step motivation-step--gap"><span>公共空间缺口</span><h3>行动安全且合规吗？</h3><p>完成任务的动作仍可能触发安全红线或干扰人群。</p></div>
            <div className="motivation-arrow"><Icon name="arrow" /></div>
            <div className="motivation-step motivation-step--response"><span>PCA-SC 回应</span><h3>决策链在哪里断裂？</h3><p>分别检查成功、安全，以及 P–C–A 信息是否相互支撑。</p></div>
          </div>
        </div>
      </section>
      ...
      <section className="section">
        <div className="container">
          <SectionHeading eyebrow="02 · 基准概览" title="论文已确认的数据规模" />
          <div className="glance-grid">
            {[["300", "标准化测试样本", "静态车站场景决策"], ["4", "每题候选动作", "互斥的高层行动"], ["5", "人类专家", "设计、标注与复核"], ["6", "已评测 MLLMs", "开源与闭源模型"], ["49", "合格参与者", "评价 16 个机器人决策案例"], ["784", "参与者×案例观察", "人类评价的原始观察数"]].map(([v, t, d]) => <article className="metric-card" key={t}><strong>{v}</strong><h3>{t}</h3><p>{d}</p></article>)}
          </div>
          <SourceNote source="PCA-SC809 摘要、§4、§5.1.4 与 §5.2.5" />
        </div>
      </section>

      <section className="section section--ink">
        <div className="container">
          <SectionHeading eyebrow="03 · 论文主要发现" title="多指标结果与人类授权判断" />
          <div className="finding-grid">
            {[
              ["01", "不同指标的最优模型不同", "P 最高为 Qwen3-VL-32B（0.968），A 最高为 GPT-4o（0.745），Safety 最高为 GPT-5.2（0.604）。"],
              ["02", "UF 是重要区分维度", "GPT-5.2 的 UF 为 0.568；其他模型为 0.041–0.127。"],
              ["03", "感知是主要错误来源", "总体错误中 Perception 占 58%，Missing key perception 为最高细分类别。"],
              ["04", "行动许可不只取决于任务完成", "社会可接受性与行动许可 ρ=.952；任务完成与行动许可 ρ=.730。"]
            ].map(([n, t, d]) => <article className="finding-card" key={n}><div className="finding-card__top"><span>{n}</span><AcademicBadge>论文结果</AcademicBadge></div><h3>{t}</h3><p>{d}</p></article>)}
          </div>
        </div>
      </section>
