...
export function OrbitExplorer({ locale = "zh" }: { locale?: "zh" | "en" }) {
  ...
  const links = [
    ...
    { type: "method" as const, label: isZh ? "评测方法" : "Methodology", kicker: isZh ? "PCA-SC Eval" : "PCA-SC Eval", summary: isZh ? "追踪安全门控、证据字段与可观察评测思维链。" : "Trace safety gates, evidence fields and the observable evaluation reasoning chain.", path: isZh ? "/zh/methodology" : "/methodology", icon: "shield" as const },
