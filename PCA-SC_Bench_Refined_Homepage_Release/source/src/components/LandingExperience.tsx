...
export function OrbitExplorer({ locale = "zh" }: { locale?: "zh" | "en" }) {
  ...
  const isZh = locale === "zh";
  const links = [
    { label: isZh ? "数据集" : "Dataset", ring: isZh ? "数据集 · DATASET · " : "DATASET · DATASET · ", path: isZh ? "/zh/dataset" : "/dataset", icon: "database" as const },
    { label: isZh ? "模型结果" : "Model Results", ring: isZh ? "模型结果 · MODELS · " : "MODEL RESULTS · ", path: isZh ? "/zh/models" : "/models", icon: "filter" as const },
    { label: isZh ? "评测方法" : "Methodology", ring: isZh ? "评测方法 · METHOD · " : "METHODOLOGY · ", path: isZh ? "/zh/methodology" : "/methodology", icon: "shield" as const },
    { label: isZh ? "评测演示" : "Evaluation Demo", ring: isZh ? "评测演示 · DEMO · " : "EVALUATION DEMO · ", path: isZh ? "/zh/demo" : "/demo", icon: "play" as const },
    { label: isZh ? "论文" : "Paper", ring: isZh ? "论文 · PAPER · 论文 · " : "PAPER · PAPER · ", path: isZh ? "/zh/paper" : "/paper", icon: "file" as const }
  ];
        ...
        {links.map((link, index) => (
          <a href={`#${link.path}`} className={`orbit-link orbit-link--${index + 1}`} aria-label={link.label} tabIndex={open ? 0 : -1} key={link.path}>
            <span className="circular-text" aria-hidden="true">
              {Array.from(link.ring).map((character, characterIndex, characters) => (
                <i
            ...
            <b><Icon name={link.icon} size={23} /></b>
          </a>
        ))}
      </nav>
